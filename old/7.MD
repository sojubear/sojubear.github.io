...
Submit Hashes: After running the sha256sum command on the downloaded files, I take the list of hashes and submit them to VirusTotal.
Analyze Results: VirusTotal provides a detailed report on each hash, including detections from various antivirus engines, behavior analysis, and any associated network activity.
Determine Maliciousness: Based on the aggregated results, I can get a good sense of what each file does and how harmful it might be without needing to manually reverse-engineer each binary.
